...
jq(document).one("ready", function() {
	...
	// Load template
	jq(document).on("click", ".templateContainer .ttile.template", function(ev) {
		// Get template map file
		var mapFile = jq(this).data("map");
		
		// Load file with ajax
		jq.ajax({
			url: "%{resources}/"+mapFile,
			dataType: "json",
			contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			crossDomain: true,
			success: function(mapContent, status, xhr) {
				// Get map content
